import { clearBag, draw, loadBag, peekSeenCount, resetBag, saveBag, } from "./bag.js";
import { Calibrator } from "./calibrate.js";
import { groupDecks, languageName, loadDeck, loadIndex, } from "./decks.js";
import { cueCorrect, cueCountdown, cuePass, cueStart, cueTick, cueTimeUp, hapticsAvailable, setSoundEnabled, unlockAudio, } from "./feedback.js";
import { fitText } from "./fit.js";
import { read, write } from "./storage.js";
import { RoundTimer } from "./timer.js";
import { DEFAULT_TILT_CONFIG, TiltReader, loadCalibration, requestMotionPermission, saveCalibration, } from "./tilt.js";
...
const dom = {
    ...
    prerondCancel: el("preround-cancel"),
...
function renderDeckGrid() {
    ...
    dom.deckGrid
        .querySelector('.deck-tile[aria-pressed="true"]')
...
function renderHome() {
    ...
    for (const button of dom.lengthGroup.querySelectorAll("button")) {
...
async function beginPreRound() {
    ...
    if (prerollWatchdog)
        window.clearTimeout(prerollWatchdog);
    prerollWatchdog = window.setTimeout(() => enableTapFallback("Calibration timed out — use the buttons."), 20000);
    if (permission !== "granted") {
        enableTapFallback(permission === "denied"
...
function startRound() {
    ...
    tilt.resume();
    nextCard();
    timer.start(settings.seconds, (remainingMs, wholeSecondsLeft, crossedSecond) => {
        ...
        if (crossedSecond && wholeSecondsLeft <= 5 && wholeSecondsLeft > 0)
            cueTick();
    }, endRound);
}
function nextCard() {
...
function handleAction(action) {
    if (!roundActive || !currentCard)
        return;
    const outcome = action === "CORRECT" ? "got" : "missed";
...
document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden" && roundActive)
        endRound();
});
// ── Boot ────────────────────────────────────────────────────────────
async function boot() {
